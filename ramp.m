...
% RAMP M-file for ramp.fig
%      RAMP, by itself, creates a new RAMP or raises the existing
%      singleton*.
%
%      H = RAMP returns the handle to a new RAMP or the handle to
%      the existing singleton*.
%
%      RAMP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RAMP.M with the given input arguments.
%
%      RAMP('Property','Value',...) creates a new RAMP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ramp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ramp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ramp

% Last Modified by GUIDE v2.5 16-Mar-2015 19:40:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ramp_OpeningFcn, ...
                   'gui_OutputFcn',  @ramp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ramp is made visible.
function ramp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ramp (see VARARGIN)

% Choose default command line output for ramp
...
% UIWAIT makes ramp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ramp_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if(isempty(get(handles.slope,'string')))
    slope = 1;
else
...
plotramp(slope , t1 , t2 , shift , choice , stp);
